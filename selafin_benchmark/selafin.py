...
class Selafin(object):
    ...
    def write(self, ds):
        ...
        nplan = 1 if self.nplan == 0 else self.nplan
        ftype, fsize = self.file["float"]
        dtype = np.float32 if fsize == 4 else np.float64
        self.append_header_slf()
        for it, time in enumerate(self.tags["times"]):
            self.append_core_time_slf(it)
            temp = np.zeros((self.nbv1, self.npoin2, nplan), dtype=dtype)
            for iv, var in enumerate(self.varnames):
                temp[iv, :] = ds.isel(time=it)[var.strip()]
            data = np.reshape(np.transpose(\
                  np.reshape(np.ravel(temp),
                             (self.nvar, self.npoin2, nplan)),
                  (0, 2, 1)),
                                            (self.nvar, self.npoin3))
            print(data.shape)
            self.append_core_vars_slf(data)

        # for iv, var in enumerate(self.varnames):
        #     print(iv, var)
        #     fig, ax = plt.subplots()
        #     ax.tricontourf(self.meshx, self.meshy, data[iv, :len(self.meshx)], cmap="jet")
        #     plt.show()
        self.fole["hook"].close()
